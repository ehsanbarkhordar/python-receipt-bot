...
import re
import datetime
from constant.messages import TxtMessages
...
def date_check(date_text):
    try:
        z = Persian(date_text).gregorian_datetime()
    except Exception:
        return False
...
def date_validation(date):
    if date and len(date) == 8:
        if date.isdigit():
            ...
            if date_check(new_date) is False:
                return TxtMessages.invalid_date
            else:
                return True

        else:
            return TxtMessages.invalid_num
    else:
        return TxtMessages.invalid_length
...
def check_dates_order(s_date, e_date):
    if int(s_date) < int(e_date):
        return True
    else:
        return TxtMessages.e_date_start_after_s_date
